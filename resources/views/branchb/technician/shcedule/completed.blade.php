...
<di class="container">
    <div class="item item-17">
        ...
        <div class="row align-items-center">
            <div class="col-8">        
                <div class="head">
                  <a href="{{ route('completed.sched.bywn') }}" style="text-decoration: none;">
                    <span class="head" style="color: black;"> Completed Services </span>
                </a>
                </div>
            </div>
            <div class="col-4">
                <form method="GET" action="{{ route('completed.sched.bywn') }}">
           ...
           <tbody> 
            @foreach ($data as $mreq)
               <tr>               
                <td>
                    {{ $mreq->id}} <br>
                   <a href="{{ route('BShowDumaRequestJHC', $mreq->id) }}"> <i class="fa-solid fa-book icons"  style="color: red;"></i></a><br>
                </td>
                   <td>{{ $mreq->name}}</td>
                   <td>{{ $mreq->address}}</td>
                   <td>{{ $mreq->phone}}</td>
